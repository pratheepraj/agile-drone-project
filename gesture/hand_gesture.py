	...
	while 1:
		...
		if len(contours) > 0:
			...
			if count_defects == 1:
				...
				if capture_flag == 1:
					cv2.putText(img, '2: Capture Image', (50, 150), cv2.FONT_HERSHEY_SIMPLEX, 4, (255, 0, 0), 20,
								cv2.LINE_AA)
				...
				if capture_flag == 0:
					cv2.putText(img, '2: Image Captured', (50, 150), cv2.FONT_HERSHEY_SIMPLEX, 4, (255, 0, 0), 20,
								cv2.LINE_AA)
				...
				if upload_flag == 1:
					...
					message = " Hi firends! Yay!!! this is my 4th tweet using api and Hand gesture. And Gesture read is 2 fingers to capture image and 4 fingers to upload tweet and image on twitter"
					image = open('NewPicture.jpg', 'rb')
